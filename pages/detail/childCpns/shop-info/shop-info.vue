<template>
	<view class='shop-info'>
		<view class='title-info'>
			<image :src="shopInfo.logo" />
			<text>{{shopInfo.name}}</text>
		</view>
		<view class='sell-info'>
			<view class='count-info sell-item'>
				<view class='total-sell count-item'>
					<view class='count'>{{shopInfo.sells}}</view>
					<view class='text'>总销量</view>
				</view>
				<view class='goods-count count-item'>
					<view class='count'>{{shopInfo.goodsCount}}</view>
					<view class='text'>全部宝贝</view>
				</view>
			</view>
			<view class='grade-info sell-item'>
				<block v-for="(item,index) in shopInfo.score" :key="index">
					<view class='grade-row'>
						<view class='item name'>{{item.name}}</view>
						<view class='item score' :class="item.isBetter ? 'good': 'less'">{{item.score}}</view>
						<view class='item better' :class="item.isBetter ? 'good': 'less'">{{item.isBetter ? '高' : '低'}}</view>
					</view>
				</block>
			</view>
		</view>
		<view class='entrance'>
			<button size='mini'>进店逛逛</button>
		</view>
	</view>
</template>

<script>
	export default {
		props: {
			shopInfo: {
				type: Object
			}
		},
		data() {
			return {

			}
		},
		methods: {

		}
	}
</script>

<style>
	.shop-info {
		padding: 16rpx 16rpx 50rpx;
		border-bottom: 12rpx solid #f2f2f2;
	}

	.title-info {
		margin-top: 30rpx;
	}

	.title-info image {
		width: 90rpx;
		height: 90rpx;
		border-radius: 46rpx;
		border: 1px solid rgba(200, 200, 200, .6);
		vertical-align: middle;
	}

	.title-info text {
		margin-left: 18rpx;
		position: relative;
		top: 6rpx;
	}

	.entrance {
		text-align: center;
	}

	.entrance button::after {
		border: none;
	}

	.entrance button {
		padding: 8rpx 100rpx;
	}

	.sell-info {
		display: flex;
		margin: 40rpx 0;
	}

	.sell-item {
		flex: 1;
		text-align: center;
	}

	.count-info {
		display: flex;
		margin: 20rpx 0;
		border-right: 1px solid rgba(200, 200, 200, .6);
	}

	.count-item {
		flex: 1;
		text-align: center;
	}

	.count-item .count {
		font-size: 40rpx;
	}

	.count-item .text {
		font-size: 28rpx;
		margin-top: 16rpx;
	}

	.grade-info {
		font-size: 28rpx;
		padding: 0 20rpx;
	}

	.grade-row {
		display: flex;
	}

	.grade-row .item {
		margin-top: 8rpx;
	}

	.grade-row .name {
		width: 200rpx;
	}

	.grade-row .score {
		flex: 1;
	}

	.grade-row .better {
		flex: 1;
	}

	.good {
		color: #ff5777;
	}

	.less {
		color: #5ea732;
	}
</style>
